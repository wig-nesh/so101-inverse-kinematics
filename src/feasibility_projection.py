...
from robot_full import lerobot_IK, lerobot_FK, create_so101, return_jacobian, manipulability
...
init_gpos = lerobot_FK(init_qpos[0:5], robot=robot)
        ...
        with lock:
            for k, direction in keys_pressed.items():
                if k in ['w', 's', 'a', 'd']:
                    ...
                    target_gpos[0] += move_x
                    target_gpos[1] += move_y
                    target_gpos[0] = np.clip(target_gpos[0], control_glimit[0][0], control_glimit[1][0])
                    target_gpos[1] = np.clip(target_gpos[1], control_glimit[0][1], control_glimit[1][1])
